...
class EquipmentCategorySerializer(serializers.ModelSerializer):
    ...
    parent = serializers.PrimaryKeyRelatedField(
        queryset=EquipmentCategory.objects.all(), required=False, allow_null=True
    )
    parent_name = serializers.CharField(source="parent.name", read_only=True)
    children_count = serializers.IntegerField(source="children.count", read_only=True)
    modules_count = serializers.IntegerField(
        source="modules_by_category.count", read_only=True
    )
...
class EquipmentPhysicalTypeSerializer(serializers.ModelSerializer):
    ...
    modules_count = serializers.IntegerField(
        source="modules_by_type.count", read_only=True
    )
