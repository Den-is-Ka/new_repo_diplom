...
class EquipmentCategory(models.Model):
    ...
    code = models.CharField(
        _("Код категории"), max_length=50, blank=True, help_text=_("Например: 1.1.1")
    )
...
class EquipmentPhysicalType(models.Model):
    ...
    image = models.ImageField(
        _("Изображение"), upload_to="equipment_types/", blank=True, null=True
    )
    order = models.PositiveIntegerField(_("Порядок отображения"), default=0)
...
class EquipmentModule(models.Model):
    ...
    price_type = models.CharField(
        _("Тип цены"), max_length=20, choices=PriceType.choices, default=PriceType.FIXED
    )
    ...
    main_image = models.ImageField(
        _("Основное изображение"), upload_to="modules/", blank=True, null=True
    )

    power_consumption = models.IntegerField(
        _("Потребляемая мощность, Вт"), blank=True, null=True
    )
    dimensions = models.CharField(_("Габариты (ШхВхГ)"), max_length=100, blank=True)
    weight = models.DecimalField(
        _("Вес, кг"), max_digits=8, decimal_places=2, blank=True, null=True
    )
...
class CompatibilityRule(models.Model):
    ...
    rule_type = models.CharField(
        _("Тип правила"), max_length=20, choices=RuleType.choices
    )
    ...
    excluded_categories = models.ManyToManyField(
        ...
        blank=True,
        help_text=_(
            "Категории, которые становятся недоступными при выборе указанной категории"
        ),
    )
...
class ContainerConfiguration(models.Model):
    ...
    height = models.CharField(
        "Высота", max_length=10, choices=Height.choices, default=Height.HEIGHT_2600
    )
    length = models.CharField(
        "Длина", max_length=10, choices=Length.choices, default=Length.LENGTH_6000
    )
    width = models.CharField(
        "Ширина", max_length=10, choices=Width.choices, default=Width.WIDTH_2500
    )
    ...
    name = models.CharField(
        "Название конфигурации", max_length=200, default="Базовая конфигурация"
    )
    description = models.TextField("Описание", blank=True)

    base_price = models.DecimalField(
        "Базовая цена контейнера", max_digits=12, decimal_places=2, default=0
    )
...
class EngineeringSystemGroup(models.Model):
    ...
    key = models.CharField(
        _("Ключ (для кода)"), max_length=64, choices=Key.choices, unique=True
    )
