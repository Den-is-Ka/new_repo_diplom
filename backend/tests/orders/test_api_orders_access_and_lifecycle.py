...
def make_user(User, username: str, company_name: str, password: str = "1234", *, is_staff: bool = False):
    """
...
def test_manufacturer_sees_all_orders(django_user_model):
    ...
    Order.objects.create(user=customer_a, order_number="ORD-T1", status=OrderStatus.NEW, total_price=1, snapshot={})
    Order.objects.create(user=customer_b, order_number="ORD-T2", status=OrderStatus.NEW, total_price=1, snapshot={})
...
def test_client_sees_only_own_orders_and_cannot_retrieve_foreign(django_user_model):
    ...
    own = Order.objects.create(user=customer, order_number="ORD-OWN", status=OrderStatus.NEW, total_price=1, snapshot={})
    foreign = Order.objects.create(user=other, order_number="ORD-FOR", status=OrderStatus.NEW, total_price=1, snapshot={})
...
def test_client_cannot_change_status(django_user_model):
    ...
    order = Order.objects.create(user=customer, order_number="ORD-CS", status=OrderStatus.NEW, total_price=1, snapshot={})
...
def test_manufacturer_lifecycle_and_invalid_transition(django_user_model):
    ...
    order = Order.objects.create(user=customer, order_number="ORD-LC", status=OrderStatus.NEW, total_price=1, snapshot={})

    c = auth(client, manufacturer)

    # NEW -> IN_REVIEW
    r = c.post(f"/api/orders/orders/{order.id}/change_status/", {"status": "IN_REVIEW", "comment": "ok"}, format="json")
    assert r.status_code == 200
    assert r.data["status"] == "IN_REVIEW"

    # IN_REVIEW -> APPROVED (quoted_at должен появиться)
    r = c.post(f"/api/orders/orders/{order.id}/change_status/", {"status": "APPROVED"}, format="json")
    assert r.status_code == 200
    assert r.data["status"] == "APPROVED"
    assert r.data["quoted_at"] is not None

    # APPROVED -> IN_PRODUCTION
    r = c.post(f"/api/orders/orders/{order.id}/change_status/", {"status": "IN_PRODUCTION"}, format="json")
    assert r.status_code == 200
    assert r.data["status"] == "IN_PRODUCTION"

    # IN_PRODUCTION -> COMPLETED (completed_at должен появиться)
    r = c.post(f"/api/orders/orders/{order.id}/change_status/", {"status": "COMPLETED"}, format="json")
    assert r.status_code == 200
